...
// whether the slug is shown. Lives inside a Dialog (see event-type-card.tsx
// / event-type-list.tsx), not in its own Card, so it renders just the form.
export function EventTypeForm({
  initial,
  titlePlaceholder = "ex: Chamada rápida",
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      ...
      <Button type="submit" disabled={saving} className="w-fit">
        {saving ? "Salvando…" : isEditing ? "Salvar" : "Criar tipo de reunião"}
      </Button>
    </form>
  );
}
